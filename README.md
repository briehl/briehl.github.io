This readme is ridiculous. Assume you're on the `source` branch, `master` gets auto-generated/published.

Steps.
1. Install with `npm install` or `yarn install`
2. Start with `yarn start`
3. Edit code in `src/`
4. Saving will auto-build locally in that `yarn start` browser page.
5. Git commit to `source` branch.
6. When happy, run `yarn deploy` to deploy build artifacts to `master`.
7. Drink beer. Be happier.
